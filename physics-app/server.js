// server.js - COMPLETE VERSION WITH USER-FRIENDLY FORMS
  ...
  try {
    ...
    if (parseInt(problemCount.rows[0].count) === 0) {
      ...
      await pool.query(`
        ...
        VALUES ('Week 1 - Kinematics', 'Basic velocity and acceleration problems', ARRAY[1, 2], '2026-01-10');
      `);
    }
...
<html>
<head>
  <title>Login - Physics Practice</title>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .demo-info code { background: #e9ecef; padding: 2px 6px; border-radius: 3px; }
  </style>
</head>
<body>
  <div class="card">
    <h1>Physics Problem Practice</h1>
    ...
    <div class="demo-info">
      ...
      Student: <code>student1</code> / <code>student123</code>
    </div>
  </div>
</body>
</html>
...
app.post('/api/problems', requireLogin, requireRole('teacher'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO problems (title, description, variables, formula, unit, tolerance_percent, max_attempts) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      [title, description, JSON.stringify(variables), formula, unit, tolerancePercent, maxAttempts]
    );
...
app.put('/api/problems/:id', requireLogin, requireRole('teacher'), async (req, res) => {
  try {
    const { title, description, variables, formula, unit, tolerancePercent, maxAttempts } = req.body;
    const result = await pool.query(
      'UPDATE problems SET title=$1, description=$2, variables=$3, formula=$4, unit=$5, tolerance_percent=$6, max_attempts=$7 WHERE id=$8 RETURNING *',
      [title, description, JSON.stringify(variables), formula, unit, tolerancePercent, maxAttempts, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post('/api/assignments', requireLogin, requireRole('teacher'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO assignments (title, description, problem_ids, due_date) VALUES ($1, $2, $3, $4) RETURNING *',
...
app.put('/api/assignments/:id', requireLogin, requireRole('teacher'), async (req, res) => {
  try {
    const { title, description, problemIds, dueDate } = req.body;
    const result = await pool.query(
      'UPDATE assignments SET title=$1, description=$2, problem_ids=$3, due_date=$4 WHERE id=$5 RETURNING *',
      [title, description, problemIds, dueDate, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/api/submissions', requireLogin, async (req, res) => {
  const subs = await pool.query(
    'SELECT * FROM submissions WHERE student_username = $1',
...
app.post('/api/submit-answer', requireLogin, requireRole('student'), async (req, res) => {
  try {
    ...
    let sub = await pool.query(
      'SELECT * FROM submissions WHERE student_username = $1 AND assignment_id = $2 AND problem_id = $3',
      [req.session.username, assignmentId, problemId]
    );
    
    if (sub.rows.length === 0) {
      // Create new submission with problem instances
      ...
      await pool.query(
        'INSERT INTO submissions (student_username, assignment_id, problem_id, problem_instances, attempts) VALUES ($1, $2, $3, $4, 0)',
        [req.session.username, assignmentId, problemId, JSON.stringify(instances)]
      );
    ...
    await pool.query(
      'UPDATE submissions SET user_answer = $1, correct_answer = $2, is_correct = $3, percent_diff = $4, attempts = attempts + 1, updated_at = CURRENT_TIMESTAMP WHERE id = $5',
      [userAnswer, correctAnswer, isCorrect, percentDiff, submission.id]
    );
...
// Teacher and Student pages (Single Page Apps)
